...
public class KThread {
	...
	public static void finish() {
		...
		// unblock parent if joined
		if (currentThread.hasJoined) {
			if (currentThread.joinParent.status == statusBlocked) {
				currentThread.joinParent.ready();
			}
		}
	...
	public void join() {
		...
		Lib.assertTrue(this != currentThread);
		Lib.assertTrue(!hasJoined);

		boolean intStatus = Machine.interrupt().disable();
		hasJoined = true;
		joinParent = currentThread;
	...
	private static void joinTest3 () {
		...
		System.out.println("call join again");
		child1.join();
	}
